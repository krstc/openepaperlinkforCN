...
---|----MAC----|--------------|--|SC|X----|Y----|--------|B1|--------|TY|
    ...
    0x09 - controller?
            ...
            0x12 - SSD (var1.6)
            0x15 - SSD (2.9 lite)
            0x18 - UC8253 (3.7 416x240 BWR)
            0x1B - SSD1677 (7.5 HD)
            0x1C - JD79665 (768x552 BWRY)
            0x19 - SSD (9.7)
            0x0A - SSD (4.2)
    ...
    */
    ...
    bool ssd1667Bwr102 = (controllerType == 0x19 && tag.solumType == STYPE_SIZE_102_SSD1667 && epdXRes == 960 && epdYRes == 640);
    ...
    if (controllerType == 0x11 && tag.solumType == STYPE_SIZE_060) {
        epd->Xres = 640;
        epd->Yres = 384;
    ...
    switch (tag.solumType) {
        ...
        case STYPE_SIZE_042:
            ...
            epd->epdMirrorV = true;
            break;
        case STYPE_SIZE_16_BWRY_HIGHRES_ROTATED:  // 0x90, used here as M3 4.2" BWRY 400x300
            tag.macSuffix = 0x9090;
            tag.OEPLtype = SOLUM_M3_BWRY_42;
            epd->epdMirrorV = true;
            break;
        case STYPE_SIZE_037_UC8253:
            tag.macSuffix = 0xD890;
            tag.OEPLtype = CUSTOM_UC8253_BWR_37;
            epd->drawDirectionRight = true;
            epd->epdMirrorH = true;
            if (!uc8253Gdey370) {
                epd->Xres = 416;
                epd->Yres = 240;
                epd->effectiveXRes = 416;
                epd->effectiveYRes = 240;
            }
            break;
        case STYPE_SIZE_043:
        ...
        case STYPE_SIZE_075_UC8159:
            ...
            tag.OEPLtype = 0x05;//CUSTOM_UC8159_BWR_75;
            if (!uc8159LowRes750 && !ssd1677HighRes750) {
                epd->Xres = epdYRes;
                epd->Yres = epdXRes;
        ...
        case STYPE_SIZE_102_SSD1667:
            ...
            if (!ssd1667Bwr102) {
                epd->Xres = 960;
                epd->Yres = 640;
                epd->effectiveXRes = 960;
                epd->effectiveYRes = 640;
            }
            break;
        case STYPE_SIZE_027:
